package com.example.krishna.onlinequiz;

/**
 * Created by Krishna on 5/23/2018.
 */

public class AddDotNetFirst {
    int id;
    String question;
    String optionf;
    String options;
    String optiont;
    String optionl;
    String answer;


    public AddDotNetFirst(int id, String question, String optionf, String options, String optiont, String optionl, String answer) {
        this.id = id;
        this.question = question;
        this.optionf = optionf;
        this.options = options;
        this.optiont = optiont;
        this.optionl = optionl;
        this.answer = answer;
    }

    public AddDotNetFirst() {

    }


    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getQuestion() {
        return question;
    }

    public void setQuestion(String question) {
        this.question = question;
    }

    public String getOptionf() {
        return optionf;
    }

    public void setOptionf(String optionf) {
        this.optionf = optionf;
    }

    public String getOptions() {
        return options;
    }

    public void setOptions(String options) {
        this.options = options;
    }

    public String getOptiont() {
        return optiont;
    }

    public void setOptiont(String optiont) {
        this.optiont = optiont;
    }

    public String getOptionl() {
        return optionl;
    }

    public void setOptionl(String optionl) {
        this.optionl = optionl;
    }

    public String getAnswer() {
        return answer;
    }

    public void setAnswer(String answer) {
        this.answer = answer;
    }



}
